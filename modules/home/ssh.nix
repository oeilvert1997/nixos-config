...
}:
{
  ...
  home.file = {
    ...
    ".ssh/id_ed25519".source =
      config.lib.file.mkOutOfStoreSymlink osConfig.sops.secrets."user/auth-ssh-key".path;

    ".ssh/id_ed25519_github".source =
      config.lib.file.mkOutOfStoreSymlink osConfig.sops.secrets."user/github-ssh-key".path;
  };
}
