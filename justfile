...
[group('system')]
diff:
    @echo "==> Building configuration for [{{ local_host }}] locally..."
    nom build .#nixosConfigurations.{{ local_host }}.config.system.build.toplevel --no-link
    @echo "==> Diffing against [{{ local_host }}]'s current system..."
    nix store diff-closures /run/current-system .#nixosConfigurations.{{ local_host }}.config.system.build.toplevel

# Usage:
#   just switch                         # Local build, local switch
#   just switch TARGET_HOST             # Local build, remote switch
#   just switch TARGET_HOST BUILD_HOST  # Build elsewhere, remote switch
# Build and activate a configuration
[group('system')]
switch target_host=local_host build_host=local_host:
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    elif [ "{{ build_host }}" = "{{ local_host }}" ]; then
...
[group('bootstrap')]
bootstrap-disko hostname ip build_on=env_var_or_default("BUILD_ON", "local"): (_validate-hostname hostname) (_validate-ip ip)
    #!/usr/bin/env bash
    set -euo pipefail
...
[group('bootstrap')]
bootstrap-install hostname ip build_on=env_var_or_default("BUILD_ON", "local"): (_validate-hostname hostname) (_validate-ip ip)
    #!/usr/bin/env bash
    set -euo pipefail
    hostname={{ quote(hostname) }}
    ip={{ quote(ip) }}
    ...
    sops --decrypt \
        ...
        "${nixos_secrets_path}/host-keys.yaml" > "$ssh_key_dir/ssh_host_ed25519_key"
